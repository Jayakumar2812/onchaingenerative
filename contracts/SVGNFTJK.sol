...
contract SVGNFTJK is ERC721URIStorage {
    uint256 public tokenCounter;
    event CreatedSVGNFT(uint256 indexed tokenId, string tokenURI);
    constructor () ERC721 ("SVG NFT_JK","svgnftJK"){
        tokenCounter =0;
    }
    function create(string memory _svg) public{
        _safeMint(msg.sender,tokenCounter);
        string memory imageURI = svgToImageURI(_svg);
        string memory tokenURIm = formatTokenURI(imageURI);
        _setTokenURI(tokenCounter,tokenURIm);
        tokenCounter +=1;
        emit CreatedSVGNFT(tokenCounter, tokenURIm);
    }
